...
def load_datasets():
    ...
    if os.path.exists(crop_file) and os.path.exists(animal_file):
        try:
            ...
            print(f"Loaded datasets: {len(crop_df)} crop records, {len(animal_df)} animal records")
    ...
    print("Generated datasets: crop_medicine_dataset.csv, animal_medicine_dataset.csv")
    return crop_df, animal_df
...
def engineer_features(df, data_type="crop", prediction_mode=False):
    ...
    common_symptoms = ['yellow', 'wilt', 'spot', 'rot', 'fever', 'diarrhea']
...
def build_model(data, type_name="crop"):
    ...
    if type_name == "crop":
        ...
        numeric_features = ['symptom_length', 'region_disease_count']
        binary_features = [col for col in data.columns if col.startswith('symptom_') or col.startswith('season_')]
    else:
        categorical_features = ['region', 'animal', 'disease', 'symptom']
        numeric_features = ['age_numeric', 'symptom_length', 'region_disease_count']
    ...
    if min_class_count < 6:
        print(f"Warning: Smallest class count ({min_class_count}) is too low. Regenerate dataset.")
        return None, None
...
def predict_medicine(input_data, crop_model_file, animal_model_file):
    ...
    for feature in features:
        if feature in input_df.columns:
            ...
            if feature in ['symptom_length', 'age_numeric', 'region_disease_count'] or feature.startswith('symptom_') or feature.startswith('season_'):
...
def main():
    ...
    if crop_model is None:
        print("Crop model failed. Please regenerate dataset.")
        return
    
    animal_model, animal_features = build_model(animal_df, "animal")
    if animal_model is None:
        print("Animal model failed. Please regenerate dataset.")
        return
